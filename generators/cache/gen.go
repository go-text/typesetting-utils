...
func (ss sizeSpec) elemType() string {
	...
	case 32:
		return "uint32"
	default:
		panic("unsupported elem size " + strconv.Itoa(v))
	}
}
...
func (ss sizeSpec) code() string {
	comment := fmt.Sprintf(`
	// %[4]s implements a cache for integer pairs,
	// mapping a key in [0,1 << %[1]d[ to a value in [0,1 << %[2]d[
	//
	// The memory layout is the following :
	//
	//	KEY 	: <%[5]d bits><%[3]d bits>
	//	VALUE 	: <%[5]d bits><%[2]d bits>
	//
	...
	//
	// This cache works best with sparse keys (sharing the same high %[5]d bits), 
	// since it handles 1 << %[3]d contiguous keys without collision.
	`, ss.keyBits, ss.valueBits, ss.cacheBits, ss.cacheType(), ss.keyBits-ss.cacheBits)
	if ss.valueBits == 0 { // set variant
		return comment + fmt.Sprintf(templateSet, ss.cacheType(), ss.elemType(),
			ss.keyBits, ss.valueBits, ss.cacheBits, ss.maxKey(), ss.maxValue(), ss.keyType(),
		)
	}
	return comment + fmt.Sprintf(templateMap, ss.cacheType(), ss.elemType(),
		ss.keyBits, ss.valueBits, ss.cacheBits, ss.maxKey(), ss.maxValue(), ss.keyType(),
	)
}

const templateMap = `type %[1]s [1 << %[5]d]%[2]s
...
const templateSet = `type %[1]s [1 << %[5]d]%[2]s
...
func (c *%[1]s) set(key %[8]s) {
	if (key>>%[3]d) != 0 { /* overflows */
		return
	}
	c.setUnchecked(key)
}
...
// assumes key < %[6]d
func (c *%[1]s) setUnchecked(key %[8]s) {
	k := key & ((1 << %[5]d) - 1)
	v := %[2]s(key >> %[5]d)
	c[k] = v
}
...
` + sizeSpec{21, 19, 8}.code() + sizeSpec{21, 0, 13}.code()
