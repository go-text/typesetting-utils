...
func (ss sizeSpec) code() string {
	if ss.valueBits == 0 { // set variant
		return fmt.Sprintf(templateSet, ss.cacheType(), ss.elemType(),
			ss.keyBits, ss.valueBits, ss.cacheBits, ss.maxKey(), ss.maxValue(), ss.keyType(),
		)
	}
	return fmt.Sprintf(templateMap, ss.cacheType(), ss.elemType(),
		ss.keyBits, ss.valueBits, ss.cacheBits, ss.maxKey(), ss.maxValue(), ss.keyType(),
	)
}
...
const templateSet = `
// %[1]s is a cache for integer keys,
// with 0 <= key < %[6]d
type %[1]s [1 << %[5]d]%[2]s
...
` + comment + sizeSpec{21, 19, 8}.code() + sizeSpec{21, 0, 13}.code()
